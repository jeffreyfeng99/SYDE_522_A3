...
BATCH_SIZE = 32
IMAGE_SIZE = 224 #227
FT_OUT_SIZE = 1792 #1792 #1280 #1792
N_EPOCH = 30

dataset_root = '../input/outofdist2'
output_root = './04032022_augmentb4_run2'
    ...
    def __init__(self, data_root, data_list=None, transform=None):
        ...
        if data_list is not None:
            ...
            for i in range(len(self.img_paths_temp)):
                if int(self.domain_labels_temp[i]) != 1:
...
# Preprocess data
def preprocess_multiple_fn(mus, stds, train=True):
    tforms = []

    for i in range(len(mus)):
        tforms.append(preprocess_fn(mu=mus[i], std=stds[i], train=train))
    
    return tforms

def preprocess_fn(mu=(0.6399, 0.6076, 0.5603), std=(0.3065, 0.3082, 0.3353), train=True):
    if train:
...
class CNNModel(nn.Module):
    ...
    def __init__(self, model_name="efficientnet_b4"):
        ...
        if model_name == "resnet18":
            ...
            self.feature = models.efficientnet_b3(pretrained=True) 
            self.feature.avgpool = nn.AdaptiveAvgPool2d(output_size=(1, 1)) # original is (7,7)
            self.feature.classifier = nn.Identity()
        else:
        ...
        self.domain_classifier.add_module('c_fc4', nn.Linear(100, 3))
...
#                                                         (0.2239, 0.2283, 0.2437)])
    img_transform_source = preprocess_fn(train=False)
    img_transform_target = preprocess_fn(mu=(0.9566, 0.9566, 0.9566), std=(0.1752, 0.1752, 0.1752), train=False)
...
#                                                         (0.2239, 0.2283, 0.2437)])
    ...
    for epoch in range(N_EPOCH):
        ...
        while i < len_dataloader:
            ...
            input_img = torch.FloatTensor(batch_size, 3, IMAGE_SIZE, IMAGE_SIZE)
            domain_label = torch.ones(batch_size) * 2.0
            ...
            class_output, domain_output = my_net(input_data=input_img, alpha=alpha)
            entropy_loss = torch.mean(torch.log(F.softmax(class_output, dim=1)))
            err_t_domain = loss_domain(domain_output, domain_label)
            err = err_t_domain + err_s_domain + err_s_label + 0.01*entropy_loss
            err.backward()
            optimizer.step()
